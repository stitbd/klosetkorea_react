...
const ProductDetailsPage = () => {
  ...
  const getImages = (img) => [img, img, img, img, img].map((src) => src || localProductImg);
    ...
    productService.getProductBySlug(slug)
      ...
      .catch(() => {
        setProduct({
          ...
          description: 'Keeping it classic with the new Puma Shuffle. Clean, white, and a touch of gold to elevate the daily grind.',
          brand: 'Puma', inStock: true, badge: 'New', category: 'Sneakers',
          colors: ['Red', 'Green', 'Yellow', 'Blue', 'Black', 'White', 'Purple', 'Orange', 'Pink'],
          ...
          <Col xs={12} md={6}>
            <div className="pdp__info">
              ...
              {product.colors?.length > 0 && (
                <div className="pdp__size-row">
                  <span className="pdp__size-label">Select Your Color: <strong>{selectedColor || ''}</strong></span>
                  <div className="pdp__sizes">
                    {product.colors.map((s) => (
                      <button key={s}
                        className={`pdp__size-btn ${selectedColor === s ? 'pdp__size-btn--active' : ''}`}
                        onClick={() => setSelectedColor(s)}>{s}</button>
                    ))}
                  </div>
                </div>
              )}
              ...
              {product.description && <p className="pdp__short-desc">{product.description}</p>}

              {product.sizes && (
                <div className="pdp__features">
                  <p>🔖 Luxurious Feel | Premium Look | Comfortable Fit</p>
              ...
              <div className="pdp__actions">
                <button className="pdp__btn pdp__btn--cart" onClick={() => addToCart(product, qty)} disabled={!product.inStock}>
                  Add to Cart
                </button>
              ...
              {/* <div className="pdp__secondary-actions">
                ...
                <a href="https://m.me/elonis.official" target="_blank" rel="noreferrer" className="pdp__action-btn pdp__action-btn--messenger">
        ...
        <div className="pdp__more-section mt-5">
          ...
          <div className="pdp__more-swiper-wrap">
            ...
            <Swiper
              ...
              loop spaceBetween={16} slidesPerView={2}
                  ...
                  <div className="pdp__related-card">
                    <Link to={`/product/${rp.slug}`}>
                      <div className="pdp__related-img-wrap">
                        <img src={rp.image} alt={rp.name} className="pdp__related-img"
                          onError={(e) => { e.target.src = PLACEHOLDER_IMG; }} />
                      </div>
                    </Link>
                    <div className="pdp__related-body">
                      <Link to={`/product/${rp.slug}`} className="pdp__related-name">{rp.name}</Link>
                      <p className="pdp__related-price">{formatPrice(rp.price)}</p>
                      <div className="pdp__related-actions">
                        <button className="pdp__related-btn pdp__related-btn--cart" onClick={() => addToCart(rp, 1)}>Add To Cart</button>
                        <Link to={`/product/${rp.slug}`} className="pdp__related-btn pdp__related-btn--buy">Buy Now</Link>
                      </div>
                    </div>
                  </div>
                </SwiperSlide>
